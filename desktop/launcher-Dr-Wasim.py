"""
GDES desktop launcher — single-user Windows build.

Double-clicking GDES.exe (or running this file) will:
...
def bootstrap_env() -> Path:
    ...
    root = app_dir()
    os.environ.setdefault("BGDDR_DATA_DIR", str(root))
    os.environ.setdefault("DJANGO_SETTINGS_MODULE", "bgddr.settings_desktop")
    # When frozen, the bundled source tree is on sys.path already; in source
    # mode make sure the project root is importable.
...
def _folder_wizard(root: Path):
    ...
    win = tk.Tk()
    win.title("GDES — Choose backup, media & update folders")
    ...
    def save():
        ...
        if not b or not m:
            messagebox.showerror("GDES", "Backup and Media folders are required."); return
        try:  # confirm we can actually create/write them
            ...
            messagebox.showerror("GDES", f"Cannot create folder:\n{exc}"); return
...
# --------------------------------------------------------------------------- #
_GITHUB_REPO = "YOUR_USERNAME/GDES"  # TODO: set after creating the repo
...
def _github_download_and_stage(manifest: dict, log=print) -> Path | None:
    ...
    try:
        ...
        # Find the folder containing GDES.exe + _internal
...
def initialise(data_dir: Path) -> None:
    ...
    run_cmd("migrate", interactive=False, verbosity=0)
    ...
    if not marker.exists():
        ...
        log("First run - seeding knowledge base ...")
...
def _admin_dialog():
    ...
    win = tk.Tk()
    win.title("GDES — Create administrator")
    ...
    def submit():
        if not u.get().strip():
            messagebox.showerror("GDES", "Username is required."); return
        if len(p.get()) < 6:
            messagebox.showerror("GDES", "Password must be at least 6 characters."); return
        if p.get() != p2.get():
            messagebox.showerror("GDES", "Passwords do not match."); return
...
def start_backups() -> None:
    ...
    log(f"Backups: startup snapshot taken; every {hours}h thereafter.")
...
def create_desktop_shortcut(data_dir: Path) -> None:
    ...
    try:
        ...
        if not desktop.is_dir():
            return
        lnk = desktop / "GDES Registry.lnk"
        ps = (
            "$s=(New-Object -ComObject WScript.Shell).CreateShortcut('{lnk}');"
            "$s.TargetPath='{exe}';"
            "$s.WorkingDirectory='{wd}';"
            "$s.IconLocation='{exe}';"
            "$s.Description='GDES Registry';"
            "$s.Save()"
...
def _status_window(server, root: Path) -> None:
    ...
    try:
        ...
        _ver = ""
    tk.Label(win, text="GDES Registry is running" + (f"  ·  v{_ver}" if _ver else ""),
    ...
    # Only offer the button when an update folder is actually configured or GitHub is available.
    if getattr(sys, "frozen", False):
        has_update_src = bool(_resolve_update_dir(root))
        if not has_update_src and not _GITHUB_REPO.startswith("YOUR_"):
            has_update_src = True
        if has_update_src:
