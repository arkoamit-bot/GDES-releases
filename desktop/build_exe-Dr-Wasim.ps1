...
try {
    ...
    $scLog = Join-Path $checkData "Logs\bgddr.log"
    if (Test-Path $scLog) { Get-Content $scLog | ForEach-Object { Write-Host "   $_" } }
} finally {
...
**Product:** GDES (Glomerular Disease Expert System)
**Version:** $appVersion
...
5) IMPORTANT
   - Keep THIS folder on the computer's local disk (e.g. C:\GDES).
