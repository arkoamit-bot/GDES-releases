# Knowledge Governance Framework -- Lupus Nephritis

**Document Version:** 1.0
**Date:** 2026-07-10
**Program:** GDES V4.1 Medical Knowledge Engineering
**Scope:** Lupus Nephritis knowledge system governance
...
| ACTIVE | Currently used in reasoning engine | Actively scoring for LN cases |
...
```python
[
  {"version": 3, "change_summary": "Added belimumab dosing per BLISS-LN trial",
   "changed_by": "Dr. Haque", "created_at": "2026-06-15T10:30:00", "diff": {...}},
  {"version": 2, "change_summary": "Updated voclosporin regimen per AURORA 2",
   "changed_by": "Dr. Rahman", "created_at": "2026-04-01T14:00:00", "diff": {...}},
  {"version": 1, "change_summary": "Initial LN rule creation",
...
| # | Check | Description | Validation Criteria | LN Status |
|---|-------|-------------|-------------------|-----------|
...
| 2 | `active_guidelines` | Active guideline sources exist | At least 1 GuidelineSource record | PASS (7 sources) |
| 3 | `active_rules_exist` | ACTIVE rules available for reasoning | At least 1 entry with status=ACTIVE | PASS (28 ACTIVE) |
...
| 7 | `rule_index` | Disease index populated | disease_id index usable | PASS (lupus_nephritis covered) |
...
### Current Health Status for LN System

```
Knowledge Platform Validation: PASSED (7/7 checks)
  tables_exist:              PASS (4 knowledge tables found)
  active_guidelines:         PASS (7 sources: KDIGO 2021/2025, EULAR 2019/2023, ACR 2021, ISN 2023, Asia-Pacific 2022)
  active_rules_exist:        PASS (28 ACTIVE rules for lupus nephritis)
  ...
  rule_index:                PASS (lupus_nephritis in disease coverage)
```
...
```
Guideline Source (e.g. KDIGO 2021 Chapter 10.3.1)
  |
  +--> KnowledgeBaseEntry (entry_id, disease_id='lupus_nephritis', rule_data)
  |       |
...
|--------------------|--------|---------|
| entry_id | KB entry | Unique identifier (e.g. KB-LN-005) |
| disease_id | Disease FK | Disease context (lupus_nephritis) |
...
5. Testing
   -> RuleTestResult created against gold-standard LN cases
...
## 8. LN-Specific Governance Considerations

### Novel Therapy Integration

| Event | Governance Action | Timeline |
|-------|-------------------|----------|
| New LN trial published (e.g., obinutuzumab long-term) | Create evidence entry; assess rule impact; propose update | Within 30 days of publication |
| Guideline update (KDIGO, EULAR, ACR) | Re-map affected rules; update guideline sources; version bump | Within 60 days of publication |
| Drug approval for LN | Expand drug knowledge; create treatment rule; validate against cases | Within 90 days of approval |
| Classification system revision | Update classification_systems field; validate against existing cases | Within 90 days |

### Class-Specific Rule Management

LN rules are organized by ISN/RPS class to support class-specific branching in clinical pathways:

| Rule Group | ISN/RPS Classes | Key Rules |
|------------|----------------|-----------|
| Proliferative LN | III, IV | KB-LN-005, KB-LN-006 (induction) |
| Membranous LN | V | KB-LN-007 (induction with voclosporin/CNI) |
| Sclerosing LN | VI | KB-LN-018 (ESKD management) |
| Minimal/Mesangial | I, II | KB-LN-004 (monitor only) |
| Maintenance (all) | Post-induction | KB-LN-008, KB-LN-009 |

### Evidence Update Priority Queue

| Priority | Evidence Source | Impact | Target Version |
|----------|----------------|--------|----------------|
| HIGH | KDIGO 2025 full LN chapter | Treatment algorithm updates | V4.1 patch |
| HIGH | NOBILITY 2-year follow-up | Obinutuzumab maintenance data | V4.2 |
| MEDIUM | BLISS-LN subgroup analyses | Belimumab in specific LN populations | V4.2 |
| MEDIUM | AURORA 2 long-term | Voclosporin durability data | V4.2 |
| LOW | Pediatric LN consensus | Age-specific protocols | V4.2 |
...
|------|---------------|-------------------|
| Knowledge Engineer | Author and maintain LN rules | Create/Edit, version, draft |
| Clinical Reviewer (Nephrologist) | Validate clinical accuracy of LN content | Review, approve, request changes |
| Clinical Director | Final authority on LN rule activation | Activate, retire |
| System Administrator | Technical infrastructure | Bootstrap validation, health monitoring |
| Quality Assurance | Test and validate LN rules | Run rule tests, validate cases |
