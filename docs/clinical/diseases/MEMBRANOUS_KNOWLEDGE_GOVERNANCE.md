# Knowledge Governance Framework — Membranous Nephropathy

**Document Version:** 1.0
**Date:** 2026-07-10
**Program:** GDES V4.1 Medical Knowledge Engineering
**Scope:** Membranous Nephropathy knowledge system governance
...
| ACTIVE | Currently used in reasoning engine | Actively scoring for MN cases |
...
```python
[
  {"version": 3, "change_summary": "Updated rituximab dosing to 1g x2 protocol from MENTOR trial",
   "changed_by": "Dr. Haque", "created_at": "2026-06-15T10:30:00", "diff": {...}},
  {"version": 2, "change_summary": "Added PLA2R tresholds for risk stratification",
   "changed_by": "Dr. Rahman", "created_at": "2026-04-01T14:00:00", "diff": {...}},
  {"version": 1, "change_summary": "Initial MN rule creation",
...
| # | Check | Description | Validation Criteria | MN Status |
|---|-------|-------------|-------------------|-----------|
...
| 2 | `active_guidelines` | Active guideline sources exist | At least 1 GuidelineSource record | PASS (5 sources) |
| 3 | `active_rules_exist` | ACTIVE rules available for reasoning | At least 1 entry with status=ACTIVE | PASS (27 ACTIVE) |
...
| 7 | `rule_index` | Disease index populated | disease_id index usable | PASS (membranous covered) |
...
### Current Health Status for MN System

```
Knowledge Platform Validation: PASSED (7/7 checks)
  tables_exist:              PASS (4 knowledge tables found)
  active_guidelines:         PASS (5 sources: KDIGO 2021/2025, Toronto 2022/2024, ISN 2023)
  active_rules_exist:        PASS (27 ACTIVE rules for membranous)
  ...
  rule_index:                PASS (membranous in disease coverage)
```
...
```
Guideline Source (e.g. KDIGO 2021 Chapter 6.4.1)
  |
  +--> KnowledgeBaseEntry (entry_id, disease_id='membranous', rule_data)
  |       |
...
|--------------------|--------|---------|
| entry_id | KB entry | Unique identifier (e.g. KB-MEMB-001) |
| disease_id | Disease FK | Disease context (membranous) |
...
5. Testing
   -> RuleTestResult created against gold-standard MN cases
...
|------|---------------|-------------------|
| Knowledge Engineer | Author and maintain MN rules | Create/Edit, version, draft |
| Clinical Reviewer (Nephrologist) | Validate clinical accuracy of MN content | Review, approve, request changes |
| Clinical Director | Final authority on MN rule activation | Activate, retire |
| System Administrator | Technical infrastructure | Bootstrap validation, health monitoring |
| Quality Assurance | Test and validate MN rules | Run rule tests, validate cases |
